...
// ══ شريط الإعلانات ══
function initAdBar() {
  const ads = JSON.parse(localStorage.getItem('omran-ads') || '[]');
  const now = new Date();
  const active = ads.filter(a => new Date(a.expires) > now);
  const featured = professionals.filter(p => p.isFeatured);
  let items = '';
  active.forEach(a => items += `<div class="ad-item">📢 <span class="tag">إعلان</span> ${a.name} — ${a.text}</div>`);
  featured.forEach(p => items += `<div class="ad-item">⭐ <span class="tag">مميز</span> ${p.name} — ${p.spec} — ${p.area||p.city||''}</div>`);
  if (!items) return; // لا تظهر الشريط لو ما في إعلانات
...
const API = 'https://omran-backend-1.onrender.com/api';
...
function renderBrowse() {
  ...
  const list = professionals.filter(p => {
    ...
    const s2 = !city || p.area.includes(city);
    const s3 = !village || p.area.includes(village);
    return s1 && s2 && s3;
  });
  ...
  return `<div class="pc" onclick="openDetail(${p.id})">
    <div style="display:flex;gap:12px;align-items:flex-start;margin-bottom:10px">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:12px;color:var(--text3)">📍 ${p.area}</div>
        ${p.userCode?`<div style="font-size:11px;color:var(--blue);font-weight:700">#${p.userCode}</div>`:''}
      </div>
...
function openDetail(id) {
  ...
  document.getElementById('detail-header').innerHTML = `
    <div style="display:flex;align-items:center;gap:14px">
      <div class="av" style="width:64px;height:64px;font-size:1.8rem;background:rgba(255,255,255,.2);color:#fff">${p.name[0]}</div>
      <div style="flex:1">
        <div style="font-size:1.2rem;font-weight:800;color:#fff">${p.name} ${p.verified?'<span style="font-size:0.7rem;background:rgba(255,255,255,.2);padding:2px 8px;border-radius:20px">✓ موثّق</span>':''}</div>
        <div style="color:rgba(255,255,255,.8);font-size:0.85rem;margin-top:3px">${p.spec} • 📍 ${p.area}</div>
        <span class="badge ${p.avail?'b-green':'b-red'}" style="margin-top:6px;display:inline-flex">${p.avail?'● متاح':'● مشغول'}</span>
      </div>
    </div>
  `;
  renderDetailBody(p);
...
function renderDetailBody(p) {
  const avg = p.reviews?.length ? (p.reviews.reduce((s,r)=>s+r.stars,0)/p.reviews.length).toFixed(1) : null;
  const reviewsHtml = p.reviews?.length ? p.reviews.map((r,i) => `
    <div class="rev-card">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
        <span style="font-weight:700;font-size:14px">${r.author}</span>
        <div style="display:flex;gap:6px">
          ${curUser ? `<button onclick="editReview('${p.id}',${i})" style="background:var(--blue-lt);color:var(--blue);border:none;border-radius:6px;padding:4px 10px;font-size:12px;font-weight:700;cursor:pointer">تعديل</button>
          <button onclick="deleteReview('${p.id}',${i})" style="background:#fee2e2;color:var(--red);border:none;border-radius:6px;padding:4px 10px;font-size:12px;font-weight:700;cursor:pointer">حذف</button>` : ''}
        </div>
      </div>
      <div class="stars">${'★'.repeat(r.stars)}</div>
      <p style="font-size:14px;margin-top:6px;color:var(--text2)">${r.text}</p>
      <p style="font-size:11px;color:var(--text3);margin-top:4px">${r.date}</p>
    </div>
  `).join('') : '<p style="color:var(--text3);text-align:center;padding:16px">لا تقييمات بعد</p>';
  
  document.getElementById('detail-body').innerHTML = `
    <div style="display:flex;gap:8px;margin-bottom:16px">
      <a href="tel:${p.phone}" class="btn-call" style="flex:1"><i class="fa-solid fa-phone"></i> اتصال</a>
      ${p.wa ? `<a href="https://wa.me/963${p.wa.slice(1)}" class="btn-wa" style="flex:1"><i class="fa-brands fa-whatsapp"></i> واتساب</a>` : ''}
    </div>
    ${p.desc ? `<div class="card"><div class="st" style="margin-bottom:8px">عن المعلم</div><p style="font-size:14px;color:var(--text2);line-height:1.6">${p.desc}</p></div>` : ''}
    <div class="card">
      <div class="st" style="margin-bottom:12px">التقييمات ${avg ? `<span style="color:var(--amber)">★ ${avg}</span>` : ''}</div>
      ${reviewsHtml}
    </div>
    ${curUser ? `
    <div class="card">
...
function renderMyPhotos() {
  ...
  grid.innerHTML = myWorkPhotos.map((src,i) => `
    <div style="position:relative">
      <img src="${src}" class="photo-thumb">
      <button onclick="deletePhoto(${i})" style="position:absolute;top:2px;right:2px;background:rgba(239,68,68,.85);color:#fff;border:none;border-radius:50%;width:20px;height:20px;cursor:pointer;font-size:10px;display:flex;align-items:center;justify-content:center">✕</button>
    </div>
  `).join('');
}
...
function renderAdmin() {
  document.getElementById('admin-list').innerHTML = professionals.map(p => `
    <div class="admin-row">
      <div style="flex:1">
        <div style="font-weight:700">${p.name}</div>
        <div style="font-size:12px;color:var(--text3)">${p.spec} • ${p.area}</div>
        <div style="margin-top:4px;display:flex;gap:5px">
          ${p.verified?'<span class="badge b-blue">✓ موثّق</span>':''}
          ${p.isFeatured?'<span class="badge b-amber">⭐ مميز</span>':''}
        </div>
      </div>
      <div style="display:flex;gap:6px">
        <button onclick="adminVerify('${p.id}')" style="background:var(--blue-lt);color:var(--blue);border:none;padding:6px 10px;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700">${p.verified?'إلغاء التوثيق':'توثيق'}</button>
        <button onclick="adminFeature('${p.id}')" style="background:#fef3c7;color:#92400e;border:none;padding:6px 10px;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700">${p.isFeatured?'إلغاء التميز':'تمييز'}</button>
      </div>
    </div>
  `).join('');
}
      ...
      data.forEach(p => {
        if (!professionals.find(x => x.phone === p.phone)) {
          professionals.unshift({
            id:p._id, name:p.name, spec:p.spec, type:p.type||'worker',
            area:p.area, phone:p.phone, wa:p.wa||p.phone,
            exp:p.exp||'', desc:p.desc||'',
            avail:p.avail!==false, verified:p.verified||false,
            isFeatured:p.isFeatured||false, reviews:[]
          });
        }
      });
      renderPros();
