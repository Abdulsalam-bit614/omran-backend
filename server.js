...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        if (userData.phone) userData.phone = normalizePhone(userData.phone);
...
function normalizePhone(phone) {
  ...
  if (p.startsWith('00')) p = p.slice(2);       // 00963 -> 963
  else if (p.startsWith('+')) p = p.slice(1);   // +963 -> 963
  else if (p.startsWith('0')) p = '963' + p.slice(1); // 09 -> 9639
  return p;
}
