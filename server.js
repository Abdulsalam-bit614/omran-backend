...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        // نجرب كل صيغ الرقم الممكنة
        const phoneVariants = [identifier, normalized];
        // إذا يبدأ برقم نضيف + في الأول
        if (/^\d/.test(identifier)) phoneVariants.push('+' + identifier);
        // إذا يبدأ بـ 0 نضيف +963
        if (identifier.startsWith('0')) phoneVariants.push('+963' + identifier.slice(1));
        // نزيل الـ + ونجرب بـ 00
        if (identifier.startsWith('+')) phoneVariants.push('00' + identifier.slice(1));
