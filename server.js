...
const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
        user: process.env.EMAIL_USER || 'omransy972@gmail.com',
        pass: process.env.EMAIL_PASS || 'ojuq rjhw zwxc zfcg'
    }
});
...
async function sendVerificationEmail(email, name) {
    ...
    await transporter.sendMail({
        from: '"عُمران" <omransy972@gmail.com>',
        to: email,
        subject: 'كود تأكيد حسابك في عُمران',
        html: `
...
app.post('/api/auth/send-code', async (req, res) => {
    try {
        ...
        console.error(e);
        res.status(500).json({ error: 'تعذر إرسال الإيميل' });
    }
});
