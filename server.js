...
app.post('/api/ai/analyze-image', async (req, res) => {
    try {
        ...
        if (!anthropicKey) return res.status(500).json({ error: 'إعداد الخدمة غير مكتمل' });
...
6. لا تخترع حرفيين غير موجودين في القائمة`;
        ...
        const response = await fetch('https://api.anthropic.com/v1/messages', {
            ...
            body: JSON.stringify({
                ...
                max_tokens: 500,
                system: systemPrompt,
                messages: messages.slice(-6) // آخر 6 رسائل فقط
            })
        });
        ...
        const data = await response.json();
        const reply = data.content?.[0]?.text || 'عذراً، تعذرت الإجابة';
        res.json({ success: true, reply });
